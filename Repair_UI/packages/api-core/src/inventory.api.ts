...
interface GetProductsParams {
  ...
  page?: number
  locationId?: string
  kind?: string                  // optional ProductKind filter — 'FinalProduct' for food POS
  includeSubCategories?: boolean // when true, also returns products from child categories
}
...
export const inventoryApi = {
  ...
  getDeviceBrands(params?: { search?: string; productCategoryId?: string; includeSubCategories?: boolean }) {
  ...
  /** Returns products linked to ANY model of the given brand — used by POS brand filter. */
  getProductsByBrand(brandId: string, companyId: string, opts?: { categoryId?: string; kind?: string; includeSubCategories?: boolean }) {
    return api.get(`/devices/brands/${brandId}/products`, {
      params: { companyId, ...opts, kind: opts?.kind ?? 'Device,Accessory,SparePart' },
    })
  },
