...
import { RepairBrandModelFilter } from './RepairBrandModelFilter'
import { RepairTicketWizard } from './RepairTicketWizard'
...
interface POSService {
  ...
  tier?: 'Universal' | 'Category' | 'ModelSpecific'
}
...
// ── renderProductGrid — avoids chained ternary + nullish-coalescing in JSX ────
...
// ── ServiceCard — single card with optional selected state ────────────────────
const ServiceCard: React.FC<{
  service: POSService
  isSelected: boolean
  onToggle: (svc: POSService) => void
  format: (n: number) => string
}> = ({ service, isSelected, onToggle, format }) => (
  <button
    onClick={() => onToggle(service)}
    className={cn(
      'relative flex flex-col items-start p-4 border rounded-xl transition-all text-left',
      isSelected
  ...
  >
    ...
    <p className="text-sm font-semibold text-gray-900 dark:text-white line-clamp-2 leading-tight pr-5">{service.name}</p>
    {service.description && (
      <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 line-clamp-1">{service.description}</p>
    )}
...
// ── TieredServicesGrid — groups services by tier ───────────────────────────────
const TieredServicesGrid: React.FC<{
  services: POSService[]
  posSearch: string
  ...
  onToggle: (svc: POSService) => void
  format: (n: number) => string
}> = ({ services, posSearch, selectedServiceCategoryId, selectedServices, onToggle, format }) => {
  ...
  // Check if we have tier info (i.e. filtered by model or category)
  const hasTiers = filtered.some(s => s.tier != null)

  if (!hasTiers) {
    // Flat grid fallback (no model selected)
    return (
      <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-3">
        {filtered.map((svc) => (
          <ServiceCard key={svc.id} service={svc} isSelected={selectedIds.has(svc.id)} onToggle={onToggle} format={format} />
        ))}
      </div>
    )
  }

  const universalSvcs  = filtered.filter(s => s.tier === 'Universal')
  const categorySvcs   = filtered.filter(s => s.tier === 'Category')
  const modelSvcs      = filtered.filter(s => s.tier === 'ModelSpecific')
  const untagged       = filtered.filter(s => s.tier == null)

  return (
    <div className="space-y-4">
      {modelSvcs.length > 0 && (
        <div>
          <div className="text-xs font-semibold text-orange-500 uppercase px-1 mb-2">For this exact model</div>
          <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-3">
            {modelSvcs.map(svc => <ServiceCard key={svc.id} service={svc} isSelected={selectedIds.has(svc.id)} onToggle={onToggle} format={format} />)}
          </div>
        </div>
      )}
      {categorySvcs.length > 0 && (
        <div>
          <div className="text-xs font-semibold text-blue-500 uppercase px-1 mb-2">For this device type</div>
          <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-3">
            {categorySvcs.map(svc => <ServiceCard key={svc.id} service={svc} isSelected={selectedIds.has(svc.id)} onToggle={onToggle} format={format} />)}
          </div>
        </div>
      )}
      {universalSvcs.length > 0 && (
        <div>
          <div className="text-xs font-semibold text-gray-400 uppercase px-1 mb-2">Works for all devices</div>
          <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-3">
            {universalSvcs.map(svc => <ServiceCard key={svc.id} service={svc} isSelected={selectedIds.has(svc.id)} onToggle={onToggle} format={format} />)}
          </div>
        </div>
      )}
      {untagged.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-3">
          {untagged.map(svc => <ServiceCard key={svc.id} service={svc} isSelected={selectedIds.has(svc.id)} onToggle={onToggle} format={format} />)}
        </div>
      )}
    </div>
  )
...
// ── ServicesGrid — kept for backward compat (unused after migration) ──────────
const ServicesGrid: React.FC<{
  services: POSService[]
  posSearch: string
  selectedServiceCategoryId: string | null
  onAdd: (svc: POSService) => void
  format: (n: number) => string
}> = ({ services, posSearch, selectedServiceCategoryId, onAdd, format }) => {
  const filtered = services.filter((s) => {
    if (posSearch && !s.name.toLowerCase().includes(posSearch.toLowerCase())) return false
    if (selectedServiceCategoryId && s.serviceCategoryId !== selectedServiceCategoryId) return false
    return true
...
export const POSPage: React.FC = () => {
  ...
  // â"€â"€ Service tab: brand/model filter (independent of product filter) â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€
  const [serviceFilterBrandId, setServiceFilterBrandId] = useState<string | null>(null)
  const [serviceFilterModelId, setServiceFilterModelId] = useState<string | null>(null)

  // â"€â"€ Selected services (multi-select mode) â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€
  const [selectedServices, setSelectedServices] = useState<POSService[]>([])
  ...
  // â"€â"€ Repair: sub-category pills â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€
  const [subCategories, setSubCategories] = useState<Array<{id: string; name: string}>>([])
  const [selectedSubCategory, setSelectedSubCategory] = useState<string | null>(null)

  // â"€â"€ Repair: brand/model chip filter (independent of CategoryFilter drill-down) â"€
  ...
  const [brandFilteredProducts, setBrandFilteredProducts] = useState<POSProduct[] | null>(null)
  ...
  // Load services — re-fetch when service filter model or category changes
  useEffect(() => {
    if (!tenant?.id) return
    setLoadingServices(true)
    ticketsApi.getServices({
      companyId: tenant.id,
      deviceModelId: serviceFilterModelId ?? undefined,
    })
      .then((res) => {
  ...
  }, [tenant?.id, serviceFilterModelId])
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        const res = await inventoryApi.getProducts({
          ...
          pageSize: 20,
          kind: isFood ? 'FinalProduct' : isRepair ? 'Device,Accessory,SparePart' : undefined,
          // Search spans all sub-categories — no need for includeSubCategories since search
          // doesn't filter by categoryId
        })
  ...
  // Load products when category / sub-category / store changes
  const loadProducts = useCallback(() => {
    if (!tenant?.id) return
    if (posSearch.trim()) return  // search results are shown instead

    // For Repair: if a brand chip filter is active, fetch via brand endpoint instead
    if (isRepair && filterBrandId) return  // handled by brand-filter effect below

    setLoadingProducts(true)
    inventoryApi
      .getProducts({
        companyId: tenant.id,
        // sub-category takes priority over top-level category
        categoryId: (selectedSubCategory ?? selectedCategory) ?? undefined,
        isActive: true,
        pageSize: 60,
        locationId: currentStoreId ?? undefined,
        // Repair: show only Device + Accessory; Food: FinalProduct; others: all
        kind: isFood ? 'FinalProduct' : isRepair ? 'Device,Accessory,SparePart' : undefined,
        // When a parent category is selected (but NO sub-cat pill chosen yet),
        // include products from child categories too.
        // e.g. "Phone" selected → show both direct phones AND Android sub-cat phones (2+4=6)
        includeSubCategories: isRepair
          && !!selectedCategory
          && !selectedSubCategory
  ...
  }, [tenant?.id, selectedCategory, selectedSubCategory, currentStoreId, posSearch, isFood, isRepair, filterBrandId, subCategories.length])

  // When top-level category changes, load sub-categories (Repair only)
  useEffect(() => {
    if (!isRepair || !selectedCategory || !tenant?.id) {
      setSubCategories([])
      setSelectedSubCategory(null)
      return
    }
    inventoryApi
      .getCategories({ companyId: tenant.id, parentCategoryId: selectedCategory })
      .then((res) => {
        const data = res.data?.data ?? res.data ?? []
  ...
  }, [selectedCategory, isRepair, tenant?.id])

  // When brand chip filter changes, fetch products by brand
  useEffect(() => {
    if (!isRepair || !filterBrandId || !tenant?.id) {
      setBrandFilteredProducts(null)
      return
    }
    setLoadingProducts(true)
    // includeSubCategories=true when on parent (no sub-cat chosen yet):
    // e.g. "Mobile" selected → finds Samsung products in Android sub-cat too
    const brandIncludeSubs = !selectedSubCategory && subCategories.length > 0
    inventoryApi
      .getProductsByBrand(filterBrandId, tenant.id, {
        categoryId: selectedSubCategory ?? selectedCategory ?? undefined,
        kind: 'Device,Accessory,SparePart',
        includeSubCategories: brandIncludeSubs || undefined,
      })
      .then((res) => {
        const raw = res.data?.data ?? res.data ?? []
        setBrandFilteredProducts(Array.isArray(raw) ? raw : [])
      })
      .catch(() => setBrandFilteredProducts([]))
      .finally(() => setLoadingProducts(false))
  }, [filterBrandId, selectedCategory, selectedSubCategory, isRepair, tenant?.id])
  ...
  const handleToggleServiceSelection = (service: POSService) => {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4rem)] -m-6">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        >
          ...
          <div className="p-4 space-y-3 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 flex-shrink-0">
            ...
            <CategoryFilter
              ...
              onCategorySelect={(id) => {
                if (contentTab === 'products') {
                  ...
                  setSelectedModelId('')
                  setModelProducts(null)
                  // Reset sub-category and brand filter on category change
                  setSelectedSubCategory(null)
                  setFilterBrandId(null)
                  setFilterModelId(null)
                  setBrandFilteredProducts(null)
                } else {
            ...
            {/* â"€â"€ Row 3: Sub-category pills (Repair only, when sub-cats exist) â"€â"€ */}
            {isRepair && subCategories.length > 0 && contentTab === 'products' && (
              <div className="flex gap-2 overflow-x-auto pb-1 px-1 scrollbar-thin">
                <button
                  onClick={() => { setSelectedSubCategory(null); setFilterBrandId(null); setFilterModelId(null); setBrandFilteredProducts(null) }}
                ...
                {subCategories.map((sc) => (
                  <button
                    key={sc.id}
                    onClick={() => { setSelectedSubCategory(sc.id); setFilterBrandId(null); setFilterModelId(null); setBrandFilteredProducts(null) }}
                    className={`px-3 py-1 rounded-full text-sm font-medium whitespace-nowrap transition-colors flex-shrink-0 ${
            ...
            {/* â"€â"€ Row 4: Brand / Model filter chips (Repair only) â"€â"€ */}
            {isRepair && contentTab === 'products' && (
              <div className="px-1">
                <RepairBrandModelFilter
                  categoryId={selectedSubCategory ?? selectedCategory}
                  includeSubCategories={!selectedSubCategory && subCategories.length > 0}
                  onBrandChange={(id) => { setFilterBrandId(id); setFilterModelId(null); setBrandFilteredProducts(null) }}
                  onModelChange={(id) => {
                    setFilterModelId(id)
                    if (id) {
                      setSelectedModelId(id)
                      setBrandFilteredProducts(null)   // model-specific parts take priority over brand-wide list
                    } else {
                      setSelectedModelId('')
                      setModelProducts(null)
                    }
          ...
          {/* Content: Products grid — priority: brand-filtered > model > category */}
          {contentTab === 'products' && renderProductGrid(
            loadingProducts,
            brandFilteredProducts || modelProducts || products,
            handleAddPOSProduct
          )}
          ...
          <div className="flex flex-col flex-1 overflow-hidden">
            {/* Service brand/model filter row */}
            <div className="px-4 pt-3 pb-2 border-b border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-900 flex-shrink-0">
              <RepairBrandModelFilter
                categoryId={null}
                onBrandChange={(id) => { setServiceFilterBrandId(id); if (!id) setServiceFilterModelId(null) }}
                onModelChange={(id) => setServiceFilterModelId(id)}
              />
            </div>
            ...
            {/* Sticky bottom bar when services are selected */}
            {selectedServices.length > 0 && (
              <div className="flex-shrink-0 sticky bottom-0 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-700 p-3 flex items-center justify-between z-10">
                <span className="text-sm font-medium text-gray-800 dark:text-gray-200">
                  {selectedServices.length} service{selectedServices.length > 1 ? 's' : ''} selected
                  {' · '}
                  Labour: {format(selectedServices.reduce((s, svc) => s + svc.basePrice, 0))}
                </span>
                <button
                  onClick={() => setTicketWizardOpen(true)}
      ...
      {/* Repair Ticket Wizard */}
      {isRepair && (
        <RepairTicketWizard
          open={ticketWizardOpen}
          onClose={() => setTicketWizardOpen(false)}
          selectedServices={selectedServices}
          deviceBrand={
            serviceFilterBrandId
              ? undefined  // brand name not tracked separately — wizard shows from pre-fill
              : undefined
          }
          deviceModel={serviceFilterModelId ?? undefined}
          deviceModelId={serviceFilterModelId ?? undefined}
          onTicketCreated={(ticketId, ticketNumber) => {
            setSelectedServices([])
